"""Function-based handler for business analyst agent."""

from langchain_core.prompts import PromptTemplate
from langsmith import traceable

from agents.subagents.business_analyst.prompts import (
...
@traceable(name="business_analyst_agent")
def business_analyst_agent(state: ProposalState, llm_instance=None) -> ProposalState:
    """Create a comprehensive business analysis based on scope and market research.

    Args:
        state: The current proposal state with refined scope
        llm_instance: Optional LLM instance (uses default if not provided)

    Returns:
        Updated state with business analysis
    """
    print("\n💼 BUSINESS ANALYST: Creating comprehensive business analysis...")
    ...
    previous_content = state.get("business_analysis", "")
    previous_content_section = ""
    if previous_content:
        previous_content_section = f"""
**PREVIOUS BUSINESS ANALYSIS CONTENT:**
...
1. **EXACT MATCHING PROCESS:**
   ...
   - Extract the KEYWORD(s) the user wants to remove (e.g., "market assessment", "roi", "risks")
...
4. **AFTER REMOVAL:**
   - Update the sections array count (e.g., if you had 6 sections and remove 1, now sections[5]:)
...
3. Update the sections array count to include the new section (e.g., if you had 6 sections, now sections[7]:)
4. Ensure the new section follows proper TOON format
5. Make the new section comprehensive and relevant to business analysis
"""
    ...
    business_analysis = chain.invoke({
    ...
    cleaned_response = clean_agent_response(business_analysis.content)

    print("✅ BUSINESS ANALYST: Business analysis complete.")

    return {
        **state,
        "business_analysis": cleaned_response,
